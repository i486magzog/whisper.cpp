...
clean:
	rm -f *.o main stream command command.keypress talk talk-llama bench quantize server lsp libwhisper.a libwhisper.so
...
command.keypress: examples/command.keypress/command.keypress.cpp examples/grammar-parser.cpp $(SRC_COMMON) $(SRC_COMMON_SDL) $(WHISPER_OBJ)
	$(CXX) $(CXXFLAGS) examples/command.keypress/command.keypress.cpp examples/grammar-parser.cpp $(SRC_COMMON) $(SRC_COMMON_SDL) $(WHISPER_OBJ) -o command.keypress $(CC_SDL) $(LDFLAGS)
