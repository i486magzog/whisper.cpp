...
std::string transcribe(
    ...
    std::string result;
// fprintf(stdout, "params.max_tokens: %d\n", params.max_tokens);
    const int n_segments = whisper_full_n_segments(ctx);
    for (int i = 0; i < n_segments; ++i) {
        const char * text = whisper_full_get_segment_text(ctx, i);
// fprintf(stdout, "i:%d, n_segments: %d, text: %s\n", i, n_segments, text);
        result += text;
...
int process_general_transcription(struct whisper_context * ctx, audio_async & audio, const whisper_params & params) {
    ...
    std::vector<float> pcmf32_cur(480000, 0.0f);
    std::vector<float> pcmf32_prompt;
    ...
    while (is_running) {
...

        {
            ...
            // std::cout << "Received: " << inputLine << std::endl;
            if (inputLine == "start") {
                is_starting = true;
                bool r = audio.clear();
            }
            else if(inputLine == "exit") { break; }

            while (is_starting && is_running){
                // delay
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                ...
                if (isDataAvailable()) {
                    ...
                    // fprintf(stdout, "%f, %f, %d, %lld\n", logprob_min, logprob_sum, n_tokens, t_ms);
                    fprintf(stdout, "{{txt}}:%s\n", txt.c_str());

                    bool r = audio.clear();
                    // fprintf(stdout, "audio.clear() = %d\n", r);
                    is_starting = false;
                    break;
                }
            }
...
bool isDataAvailable() {
    ...
    timeval timeout;
    timeout.tv_sec = 0; // 0초 대기
    timeout.tv_usec = 0; // 0마이크로초 대기

    // select 함수는 readfds 중 하나라도 읽을 준비가 되면 1을 반환합니다.
...
int main(int argc, char ** argv) {
    ...
    // print some info about the processing
    {
        ...
        if (!whisper_is_multilingual(ctx)) {
            if (params.language != "en" || params.translate) {
                ...
                fprintf(stderr, "{{warning}}:%s: WARNING: model is not multilingual, ignoring language and translation options\n", __func__);
            }
        }
    }
    ...
    if (!audio.init(params.capture_id, WHISPER_SAMPLE_RATE)) {
        fprintf(stderr, "{{error}} %s: audio.init() failed!\n", __func__);
        return 1;
    }
